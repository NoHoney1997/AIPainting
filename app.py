...
TARGET_COLLECT_PROMPTS = {
    "S0a": "虚构角色的姓名、性别、专业、年级等基本信息，可含一点性格或特点。",
...
def generate_target_guide(target_id: str, context: str = None) -> str:
    ...
    return template.replace("{name}", name).replace("{summary}", summary)
...
def _init_comic_frames():
    ...
    for i, _ in enumerate(default_frames):
        comic_data["frames"].append({
            "frame_index": i + 1, "description": "", "versions": [],
            "current_version": 0, "final_version": 0, "final_path": "", "image_path": "",
        })
    if situation == "A":
...
【提取】
从用户输入中提取：name, gender, major, grade。"""
...
只返回JSON：
{{"is_sufficient": true/false, "follow_up_needed": true/false, "follow_up_message": "追问语（仅当 is_sufficient=false 时）", "extracted_data": {{"name":"", "gender":"", "major":"", "grade":""}}}}"""
...
{{"is_sufficient": true/false, "follow_up_needed": true/false, "follow_up_message": "追问语（仅当 is_sufficient=false 时）", "extracted_data": {{"appearance": "完整外貌描述"}}}}"""
    ...
    try:
        ...
        # 降级：S0a 需要四项，S0b 需要长度 >= 8 且描述性内容多
        stripped = user_input.strip()
        if stage == "S0a":
            name = st.session_state.get("character_name", "")
            gender = st.session_state.get("character_gender", "")
            major = st.session_state.get("character_major", "")
            grade = st.session_state.get("character_grade", "")
            is_sufficient = bool(name and gender and major and grade)
        else:
...
WELCOME_QUESTION_HINT = "就是一起创作一个虚构的大学生角色，编一个TA的故事。完全匿名，不用有压力。准备好了随时告诉我~"
